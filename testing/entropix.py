# Graciously adapted from https://github.com/EdwardDali/EntropixLab
...
def _sample(
    logits: np.ndarray, temperature: float, top_p: float, top_k: int, min_p: float
) -> int:
    logits_tensor = torch.from_numpy(logits)
...
def sample(
    logits: np.ndarray, cfg: SamplerConfig, input_ids: List[int]
) -> Tuple[int, SamplerState]:
    """
    Sample a token from the logits using the specified sampling strategy.
    """
    entropy, varentropy = calculate_varentropy_logsoftmax(logits)

    if do_argmax:
        return np.argmax(logits), SamplerState.ARGMAX
    ...
    if state == SamplerState.ARGMAX:
        ...
        # TODO: add a different token for COT
...
def apply_repetition_penalty(
    logits: np.ndarray, input_ids: List[int], penalty: float
) -> np.ndarray:
...
class VarentropyLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids: List[int], scores: List[float]) -> List[float]:
        ...
        if self.check_ngram_repetition(input_ids):
            # Increase temperature and top_k to encourage diversity when repeating n-grams
    ...
    def check_ngram_repetition(self, input_ids: List[int]) -> bool:
        for n in range(2, self.config.max_ngram_size + 1):
            ngrams = [
                tuple(input_ids[i : i + n]) for i in range(len(input_ids) - n + 1)
            ]
...
def generate_response(model, prompt, max_tokens=1000):
    ...
    # keeps track of the strategies used
    logits_processors = LogitsProcessorList([logits_processor])  # llama-cpp-python tool
    # https://github.com/abetlen/llama-cpp-python/blob/7c4aead82d349469bbbe7d8c0f4678825873c039/llama_cpp/llama.py#L2383
    ...
    # model forward pass. Where we do the various sampling.
    stream = model(
        prompt,
...
def get_user_input():
    while True:
        ...
        if prompt.lower() == "quit":
            return None
        if prompt:
            system_prompt = "You a helpful assistant."
            # <|begin_of_text|> is default for llama cpp python
            llama_prompt_full = f"""<|start_header_id|>system<|end_header_id|>
...
"""
            picard_prompt_full = f"""<|im_start|>system
...
"""
            ...
            return picard_prompt_full
        print("Please enter a non-empty prompt.")


def main():
    model_path = "picard-merged.gguf"  # "Llama-3.2-1B-Instruct-Q4_0.gguf"  # Replace with your GGUF model path
